...
function Header() {
  ...
  const admin =  cookieuser.adminEmail 
                  ...
                  <div className={ cx(styles.collg8, styles.colmd9, styles.textRight) }>
                    ...
                    <div style={{display: state2===2 ? 'block' : null}} className={styles.menu}>
                      <ul className={styles.dInlineBlock}>
                        <li><Link2 style={{textDecoration: 'none'}}  href={admin ? '/admin/export' : '/dashboard'}><a style={{textDecoration: 'none'}} >{admin ? 'Export' : 'Dashboard'}</a></Link2></li>
                      </ul>
                      {admin ? <ul className={styles.dInlineBlock}>
                        <li><Link2 style={{textDecoration: 'none'}}  href='/admin/profile'><a style={{textDecoration: 'none'}} >Dashboard</a></Link2></li>
                      </ul> : null}
                      <div className={cx (styles.signin, styles.dInlineBlock)}>
                        <Link2 href={user ? '/api/logout' : '/'} ><a className={styles.btn}>{user || admin ? 'Logout' : 'Sign in'}</a></Link2>
                      </div>
                    </div>
                  </div>
                </div>
